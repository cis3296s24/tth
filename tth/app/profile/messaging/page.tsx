      ...
      if (user) {
        ...
        onValue(messageRef, (snapshot) => {
          if (snapshot.exists()) {
            ...
            const AllMessages: Message[] = MessageData ? Object.values(MessageData) : [];
            const filteredItems = AllMessages.filter(item => item.User_from === user.uid || item.User_to === user.uid);
  ...
  return (
    <div className="space-y-6">
      ...
      {messages && messages.sort((a, b) => parseInt(a.ID) - parseInt(b.ID)).map((message, index) => (
            <div key={index} className={'flex justify-between items-center bg-gray-300 text-black rounded-lg p-2 my-1 mr-auto'}>
              <span>{message?.ID.substring(0, 4)}...${message?.ID.substr(message?.ID.length - 4)}</span> 

              <button onClick={e=>messagehandel(message?.ID)}
